...
LastEditTime: 2024-01-19 16:55:59
...
if __name__ == '__main__': 
    ...
    GymEnv = gym.make('GymCelluarEnv-v1', cellularEnv=env, ser_cat_action_space=action_space, qoe_weight=qoe_weight, se_weight=se_weight,total_timesteps=total_timesteps)
    ...
    GymEnv.save_results()
    plt.clf()
    rewards = np.array(GymEnv.rewards)
    utilitys = np.array(GymEnv.utilitys)
    plt.title('Training...')
    plt.xlabel('Episode')
    plt.ylabel('Duration')
    plt.plot(utilitys)
